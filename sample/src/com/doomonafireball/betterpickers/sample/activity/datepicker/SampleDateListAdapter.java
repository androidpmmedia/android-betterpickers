...
public class SampleDateListAdapter extends BaseSampleActivity {
  ...
  private class SampleAdapter extends BaseAdapter
    ...
    private DateTimeFormatter mDateTimeFormatter =
        new DateTimeFormatterBuilder().appendMonthOfYearShortText()
            .appendLiteral(" ")
            .appendDayOfMonth(1)
            .appendLiteral(", ")
            .appendYear(4, 4)
    ...
    public SampleAdapter(Context context, FragmentManager fm) {
      ...
      for (int i = 1; i < 13; i++) {
        DateTime dt =
            new DateTime().withMonthOfYear(i).withDayOfMonth(1).withYear(now.year().get() - 1);
        mDateTimes.add(dt);
      }
      for (int i = 1; i < 13; i++) {
        DateTime dt =
            new DateTime().withMonthOfYear(i).withDayOfMonth(1).withYear(now.year().get());
        mDateTimes.add(dt);
      }
      for (int i = 1; i < 13; i++) {
        DateTime dt =
      ...
      mDatePickerBuilder = new DatePickerBuilder().setFragmentManager(fm)
    ...
    public void onDialogDateSet(int reference, int year, int monthOfYear, int dayOfMonth) {
      DateTime dt =
          new DateTime().withMonthOfYear(monthOfYear + 1).withDayOfMonth(dayOfMonth).withYear(year);
