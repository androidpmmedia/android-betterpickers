...
public class SimpleMonthView extends View {
  ...
  public SimpleMonthView(Context context) {
    ...
    mRowHeight =
        (res.getDimensionPixelOffset(R.dimen.date_picker_view_animator_height) - sMonthHeaderSize)
            / MAX_NUM_ROWS;
   ...
   * number. Other
   * parameters have a default value and will only update if a new value is included, except for
   * focus month, which
   * will always default to no focus month if no value is passed in. See {@link
   * #VIEW_PARAMS_HEIGHT}
   * for more info on
   * parameters.
   *
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    setMeasuredDimension(MeasureSpec.getSize(widthMeasureSpec),
        mRowHeight * mNumRows + sMonthHeaderSize);
  }
  ...
  /**
   * Draws the week and month day numbers for this week. Override this method if you need different
   * placement.
   *
  ...
  /**
   * Calculates the day that the given x position is in, accounting for week number. Returns a Time
   * referencing that
   * day or null if
   *
  ...
  private class MonthViewNodeProvider extends TouchExplorationHelper<CalendarDay> {
    ...
    /**
     * Generates a description for a given time object. Since this description will be spoken, the
     * components are
