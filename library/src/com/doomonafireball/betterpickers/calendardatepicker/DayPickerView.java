...
public class DayPickerView extends ListView implements OnScrollListener, OnDateChangedListener {
  ...
  /**
   * Creates a new adapter if necessary and sets up its parameters. Override this method to provide
   * a custom adapter.
   */
  ...
  /**
   * This moves to the specified time in the view. If the time is not already in range it will move
   * the list so that
   * the first of the month containing the time is at the top of the view. If the new time is
   * already in view the list
   * will not be scrolled unless forceScroll is true. This time may optionally be highlighted as
   * selected as well.
   *
  ...
  private SimpleMonthAdapter.CalendarDay findAccessibilityFocus() {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (child instanceof SimpleMonthView) {
        final SimpleMonthAdapter.CalendarDay focus =
            ((SimpleMonthView) child).getAccessibilityFocus();
        if (focus != null) {
