...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
  ...
  public void initialize(Context context, int initialHoursOfDay, int initialMinutes,
    ...
    for (int i = 0; i < 12; i++) {
      hoursTexts[i] =
          is24HourMode ? String.format("%02d", hours24[i]) : String.format("%d", hours[i]);
   ...
   * selectable area to each
   * of the 12 visible values, such that the ratio of space apportioned to a visible value : space
   * apportioned to a
   * non-visible value will be 14 : 4. E.g. the output of 30 degrees should have a higher range of
   * input associated
   * with it than the output of 24 degrees, because 30 degrees corresponds to a visible number on
   * the clock circle (5
   ...
   * the degrees
   * corresponding to visible numbers (i.e. those divisible by 30) will be weighted heavier than
   * the
   * degrees
   * corresponding to non-visible numbers. See {@link #preparePrefer30sMap()} documentation for the
   * rationale and
   ...
   * be
   * allowed to snap
   * to whichever is closer. Use 1 to force strictly higher, -1 to force strictly lower, and 0 to
   * snap to the closer
   * one.
   ...
   * selection (i.e. minutes),
   * force the selection to one of the visibly-showing values.
   * @param forceDrawDot The dot in the circle will generally only be shown when the selection is
   * on
   * non-visible
   ...
   * array here, inside
   * which the value will be true if the selection is in the inner circle, and false if in the
   * outer
   * circle.
  ...
  /**
   * Try to vibrate. To prevent this becoming a single continuous vibration, nothing will happen if
   * we have vibrated
   * very recently.
   */
